...
export default function HomePage() {
  ...
  React.useEffect(() => {
    Axios.get(
      ...
      const idArea = rs.data.area;
      Axios.get(`${process.env.REACT_APP_GETONEAREABYID}?id=${idArea}`).then(
        (rs) => {
          setCurrentArea({ id: idArea, nameArea: rs.data.nameArea });
        }
      );
    });
  }, []);
  ...
  const Add = (e) => {
    ...
    const id = uuid();
    Axios.get(
      `${process.env.REACT_APP_ADDEMPLOYEE}?id=${id}&fullname=${newEmployee.fullname}&address=${newEmployee.address}&phone=${newEmployee.phone}&isDeleted=${isDeleted}&area=${newEmployee.area}`
    )
      .then((rs) => {
